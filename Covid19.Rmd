...
# por edades, países y la reacción tomada por cada uno de ellos
# en tiempo y forma.
...
## Veamos a continuación la evolución en la Comunidad de Madrid.
```{r global, warning=FALSE, include=FALSE}
covar <- read_csv("Data Science/Covid19/Kaggle/ccaa_covid19_fallecidos_long.csv")
covar$fecha <- as.Date( covar$fecha, "%d/%m/%Y")
...
```{r fallecimientos, warning=FALSE}
ggplot(covsp,aes(x=fecha,y=Fallecimientos.nuevos),na.rm = TRUE)+geom_line()+theme_tufte()
```
```{r edadrango , warning=FALSE}

covedad <- read_csv("Data Science/Covid19/Kaggle/nacional_covid19_rango_edad.csv")

covedad$fecha <- as.Date( covedad$fecha, "%m/%d/%Y")
sex <- "ambos sexos"
tr <- covedad %>% group_by(rango_edad)%>% filter(sexo %in% sex & rango_edad != "Total" )

 r <- tr %>% ggplot(aes(rango_edad, fallecidos, group=1))+
  geom_line() + 
   labs(title =  "Máximos fallecidos diarios por rango de edades")
...
h <- tr  %>% ggplot(aes(rango_edad, fallecidos, group=1))+
  geom_line()+ 
  labs(title =  "Máximos fallecidos diarios hombres por rango de edades")
 
 
sex <- "mujeres"
tr <- covedad %>% group_by(rango_edad)%>% filter(sexo %in% sex & rango_edad != "Total" )

m <- tr %>% ggplot(aes(rango_edad, fallecidos, group=1))+
  geom_line()+ 
  labs(title =  "Máximos fallecidos diarios mujeres por rango de edades")
 
 
 grid.arrange(h, m, r, ncol = 1)
...
### `r paste0("Total Confirmed Cases : ", max(corona$ObservationDate))` 
...
corona_latest$`Country/Region`[corona_latest$`Country/Region`  == "UK"] = "United Kingdom of Great Britain and Nothern Ireland"
corona_latest %>% 
  janitor::clean_names() %>% 
...
highchart() %>%
  ...
  hc_subtitle(text = 'con Log del Total Confirmados') 
```
...
corona %>% 
  ...
  hc_title(text = "Tendencia lineal de COVID-19 confirmados max país") %>% 
...
corona %>% 
  ...
  hchart("line",hcaes(x = ObservationDate, y = total_count)) %>% 
  hc_title(text = "Tendencia lineal Covid 19 Recuperados max por país") %>% 
  hc_add_theme(hc_theme_538())
```
...
corona %>% 
  ...
  hchart("line",hcaes(x = ObservationDate, y = total_count)) %>% 
  hc_title(text = "Tendencia lineal Covid19 fallecidos max país") %>% 
  hc_add_theme(hc_theme_538())
...
China_plot<- ggplot(China, aes(x= Date, y= Count ,fill = Cases, color = Cases , group=Cases ))+
           ...
           theme( plot.title = element_text(hjust = 0.5 , face = "bold", size = 25),
                 text = element_text(hjust = 1,face = "bold", size = 25),
...
map_death <- world +
  ...
  labs(size = 'Cases Count')+   theme( plot.title = element_text(hjust = 0.5 , face = "bold", size = 25),
         text = element_text(hjust = 1,face = "bold", size = 20))
map_death
```
### Por confirmados
...
Links
====================
### Gracias especialmente a datadista por mantener base de datos actualizada del Ministerio de Sanidad.(https://github.com/datadista/datasets/tree/master/COVID%2019)
